...
class VariablesRetriever
{
	...
	macro public static function buildFileReferences(aFile:String):Array<Field>
	{ 
		var code:String;
		var exp:Array<Expr> = new Array();
		
		var aFile=File.read(aFile).readAll();
		var fileAsString:String = aFile.getString(0, aFile.length);
		...
		while (counter>=0)
		{
			...
			fields.push({
				...
				#end
				kind: FieldType.FVar(macro:Int, macro $v{ Std.parseInt(parts[1])}),
